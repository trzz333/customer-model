      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
        <aside className="no-print rounded-xl border border-card-border bg-card p-5 h-fit lg:sticky lg:top-6">
          ...
          <details className="mt-4 border-t border-card-border pt-3">
            ...
            <p className="text-xs text-muted-fg my-2">Applied to every world. λ drives the outcome wherever your moves create a perceived loss; with no loss to weight, it does little. It&apos;s held constant across worlds as the shared science; what changes between worlds is who the customers are.</p>
        ...
        <section>
          ...
          <div id="result-printable">
            ...
            <div className="rounded-xl border border-card-border bg-card-muted p-5 text-sm text-muted-fg leading-relaxed">
              ...
              <p>Deterministic by design, not a committee of LLM personas answering as fake customers: same seed, same result, every run. This is a seeded simulation of many individual customers, so the seed fixes the whole roll (who is in the crowd, who reads each round as a loss, who actually leaves), not only the perception-noise misreads. A different seed is an equally valid roll and can move the per-100 count by several points, so read the spread across seeds and worlds, not the single number. Counts are shown per 100 starting customers; the engine runs a larger population and rescales for readability, and word-of-mouth can push the ending count above 100 because it acquires as well as loses.</p>
            </div>
          </div>
        </section>
      </div>
